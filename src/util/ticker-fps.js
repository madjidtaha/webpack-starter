export default class TickerFPS {

    constructor(fps, handler){

        this._handleSynch = this._handleSynch.bind(this);
        ...
        */
        ...
        this._timerId = null;
        this._now = window.performance && (performance.now || performance.mozNow || performance.msNow || performance.oNow || performance.webkitNow);
        this._times = [];
        this._tickTimes = [];
    ...
    getMeasuredTickTime(ticks) {
        ...
        ticks = Math.min(times.length, ticks || (this._fps | 0 ));
    ...
    getMeasuredFPS(ticks) {
        ...
        ticks = Math.min(times.length - 1, ticks || (this._fps | 0));
    ...
    */
    ...
    _tick() {
        ...
        // dispatch event
        var event = {};
        var maxDelta = this._maxDelta;
        event.delta = (maxDelta && elapsedTime > maxDelta) ? maxDelta : elapsedTime;
        event.time = time;
        event.step = Math.round(event.delta / (1000 / this._fps));
        this._handler(event);
    }

    _getTime() {
        return ((this._now && this._now.call(performance)) || (new Date().getTime())) - this._startTime;
    }
